...
/**
 * Records every outbound HTTP call and serves canned responses, so no real
 * network traffic is made. sendInvoice()/downloadInvoiceDocument() reach the
 * HTTP adapter directly (via $this->http), the other calls go through the
 * protected request() wrapper — both funnel into $requestLog here.
 */
...
class SuperPdpClientTest extends TestCase
{
    ...
    #[\PHPUnit\Framework\Attributes\Test]
    public function it_uploads_the_invoice_pdf_as_a_raw_application_pdf_body(): void
    {
        /* Arrange */
        $provider = new FakeSuperPdpClient([
            ['success' => true, 'status' => 'sent', 'message' => 'ok', 'http_code' => 200, 'request' => [], 'response' => ['id' => 'sp-inv-1']],
        ]);
        $provider->authenticate($this->defaultSettings());
        $tmp = tempnam(sys_get_temp_dir(), 'inv') . '.pdf';
        file_put_contents($tmp, '%PDF-1.4');
        ...
        $call = $provider->requestLog[0];
        $this->assertSame(RequestMethod::POST, $call['method']);
        $this->assertStringContainsString('/v1.beta/invoices', $call['url']);
        $this->assertSame('%PDF-1.4', $call['options']['body']);
        $this->assertContains('Content-Type: application/pdf', $call['options']['headers']);
    }

    #[\PHPUnit\Framework\Attributes\Test]
    public function it_adds_the_disable_pre_check_flag_to_the_request_query(): void
    {
        /* Arrange */
        $settings                      = $this->defaultSettings();
        $settings['disable_pre_check'] = true;
        $provider                      = new FakeSuperPdpClient([
            ['success' => true, 'status' => 'sent', 'message' => 'ok', 'http_code' => 200, 'request' => [], 'response' => ['id' => 'sp-inv-2']],
        ]);
        $provider->authenticate($settings);
        $tmp = tempnam(sys_get_temp_dir(), 'inv') . '.pdf';
        file_put_contents($tmp, '%PDF-1.4');
